...
sqrt_n_in_csv_order = [0, 1, 2, 3, 5, 6, 7, 9, 10, 11, 12, 13, 14, 15]
...
draw_performance_graph([0, 12, 2, 5, 6, 7, 13, 1, 10, 14, 11, 3, 9, 15], "Integer SQRT performance on 6502 (all solutions)", "result_all.svg")
...
data = [
    ...
    ["sqrt13", 141,  265.4, True],
    ["sqrt14", 224,  198.1, True],
...
for entry in data:
    ...
    if entry[0] == "sqrt7":
        ax.annotate(entry[0], xy=(entry[2] + 10, entry[1] - 15), color=color)
    elif entry[0] == "sqrt12":
        ax.annotate(entry[0], xy=(entry[2] - 30, entry[1] + 15), color=color)
    else:
        ax.annotate(entry[0], xy=(entry[2] + 10, entry[1]), color=color)
